...
async def sentiment_start(message: types.Message, state: FSMContext):
    ...
    await message.answer("Отправьте текст, и я проверю его на токсичность через RabbitMQ:")

@router.message(BotStates.waiting_for_sentiment_text)
async def sentiment_analyze(message: types.Message, state: FSMContext, ml_client: MlClient):
    ...
    try:
        ...
        details = "\n".join([f"- {k}: {'❌' if v else '✅'}" for k, v in preds.items() if k != 'normal'])
